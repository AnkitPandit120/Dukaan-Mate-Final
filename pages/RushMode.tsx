...
const RushMode: React.FC = () => {
    ...
    const handleTranscription = async (audioBlob: Blob) => {
        ...
        try {
            ...
            - Price Handling: Listen carefully for price indicators in Hinglish such as 'rupay', 'rupees', 'rs', 'bucks', 'ka', 'wala', 'at', 'for'. Price is in Indian Rupees (₹). Limit price to 2 decimal places.
            - Ambiguity Detection:
            ...
            - Structure: Return a RAW JSON array of objects: { "name": string, "quantity": number, "price": number | null, "isPriceAmbiguous": boolean }.
            - Missing Price: If price is completely absent, set 'price' to null.
            - NO Markdown: Do not wrap the output in markdown code blocks. Return only the raw JSON string.
            ...
            try {
                ...
                // Aggressive cleaning to remove code blocks and extraneous text
            ...
            if (Array.isArray(parsedJson)) {
                const newItems: ParsedItem[] = parsedJson.map(item => ({
                    ...
                    price: typeof item.price === 'number' ? parseFloat(item.price.toFixed(2)) : null,
    ...
    const totalValue = capturedItems.reduce((sum, item) => sum + ((item.price || 0) * item.quantity), 0);
            ...
            <div className="mt-8 max-w-4xl mx-auto bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm">
                ...
                {capturedItems.length > 0 && (
                    <div className="mt-6 text-right">
                        ...
                        <button onClick={initiateAddItems} disabled={!allItemsValid} className="bg-green-600 text-white py-2 px-6 rounded-md shadow-sm hover:bg-green-700 disabled:bg-green-400 disabled:cursor-not-allowed flex items-center justify-center ml-auto transition-colors">
            ...
            {isConfirmModalOpen && (
                <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center p-4">
                    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-sm">
                        <h3 className="text-lg font-bold text-gray-800 dark:text-gray-100 mb-4">Confirm Sales</h3>
                        <p className="text-gray-600 dark:text-gray-300 mb-2">
                            You are about to add <strong>{capturedItems.length}</strong> items to your sales record.
                        </p>
                         <p className="text-gray-800 dark:text-gray-200 font-semibold mb-6">
                            Total Value: ₹{totalValue.toFixed(2)}
                        </p>
                        <div className="flex justify-end space-x-4">
                            <button onClick={() => setIsConfirmModalOpen(false)} className="bg-gray-200 text-gray-800 py-2 px-4 rounded-md hover:bg-gray-300 dark:bg-gray-600 dark:text-gray-200 dark:hover:bg-gray-500">
                                {t('cancel')}
                            </button>
                            <button onClick={confirmAddItems} className="bg-green-600 text-white py-2 px-4 rounded-md hover:bg-green-700">
                                Confirm Add
                            </button>
                        </div>
                    </div>
