...
const Dashboard: React.FC = () => {
  const { sales, expenses, stock, payments, addFeedback } = useData();
  const { t } = useLanguage();
  ...
  const stats = useMemo(() => {
    ...
    // Ensuring total profit is at least 0 for better UX
  ...
  const smartInsights = useMemo(() => {
      return {
          ...
          expiringSoon: stock.filter(item => item.expiryDate && isNearExpiry(item.expiryDate)),
      }
  }, [sales, stock]);
  ...
  const profitExpenseData = [
      { name: t('totalProfit'), value: stats.totalProfit },
  ...
  const handleFeedbackSubmit = (e: React.FormEvent) => {
      e.preventDefault();
      addFeedback({ rating: feedbackRating, comment: feedbackComment });
  ...
  return (
    <div className="space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              ...
              <div className="space-y-8">
               ...
               <div className="space-y-8">
                  ...
                  {/* New Section: Expiring Soon */}
                  <div>
                      <h4 className="font-semibold text-gray-600 dark:text-gray-300 mb-3">{t('nearExpiry')}</h4>
                      <ul className="space-y-2">
                          {smartInsights.expiringSoon.length > 0 ? smartInsights.expiringSoon.map(item => (
                              <InsightItem 
                                key={item.id} 
                                item={item.itemName} 
      ...
      {/* Footer */}
      <footer className="mt-12 pt-6 border-t border-gray-200 dark:border-gray-700 flex flex-col md:flex-row justify-between items-center text-sm text-gray-500 dark:text-gray-400 pb-8">
        <p>&copy; {new Date().getFullYear()} DukaanMate. All rights reserved.</p>
        <div className="flex space-x-6 mt-4 md:mt-0 items-center">
          <a href="#" className="hover:text-blue-600 dark:hover:text-blue-400 transition-colors">Privacy Policy</a>
          <a href="#" className="hover:text-blue-600 dark:hover:text-blue-400 transition-colors">Terms of Service</a>
          <button onClick={() => setIsFeedbackModalOpen(true)} className="hover:text-blue-600 dark:hover:text-blue-400 transition-colors font-semibold">Send Feedback</button>
        </div>
      </footer>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center p-4">
              <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-md">
                  <div className="flex justify-between items-center mb-4">
                      <h3 className="text-lg font-bold text-gray-800 dark:text-gray-100">Submit Feedback</h3>
                      <button onClick={() => setIsFeedbackModalOpen(false)} className="text-gray-500 hover:text-gray-700 dark:hover:text-gray-300">
                          ...
                          <div className="flex space-x-2">
                              {[1, 2, 3, 4, 5].map(r => (
                                  <button 
                                      ...
                                      className={`p-2 rounded-full transition-colors ${feedbackRating >= r ? 'text-yellow-400' : 'text-gray-300'}`}
                                  >
                                      <Icons.Zap className="h-6 w-6 fill-current"/>
                                  </button>
                              ))}
                          </div>
                      </div>
