...
import { Sale, Expense, StockItem, Payment, Feedback } from '../types';
...
interface DataContextType {
  ...
  addFeedback: (feedback: Omit<Feedback, 'id' | 'date' | 'userId' | 'userName'>) => void;
}
...
export const DataProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const addFeedback = (feedbackData: Omit<Feedback, 'id' | 'date' | 'userId' | 'userName'>) => {
        if (!user) return;
        ...
        // Here, we simulate a global feedback store in localStorage so Admin can see it.
        const globalFeedback: Feedback[] = JSON.parse(localStorage.getItem('dukaan-feedback') || '[]');
        globalFeedback.unshift(newFeedback);
        localStorage.setItem('dukaan-feedback', JSON.stringify(globalFeedback));
    };

    return (
        <DataContext.Provider value={{ sales, addSale, expenses, addExpense, stock, updateStock, addStockItem, updateStockItem, deleteStockItem, bulkAddStock, payments, addPayment, updatePaymentStatus, online, qrCode, setQrCode, addFeedback }}>
            {!isLoading && children}
